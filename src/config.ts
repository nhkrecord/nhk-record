...
const config = yargs(process.argv.slice(2))
  .option('b', {
    alias: 'safety-buffer',
    describe:
      'Number of extra milliseconds to record before and after scheduled airtime',
    type: 'number'
  })
  .option('c', {
    alias: 'config',
  ...
  .option('m', {
    alias: 'match-pattern',
    describe:
      'Glob pattern of desired program name (can be used multiple times)',
    type: 'string',
    array: true
  })
  .option('s', {
    alias: 'schedule-url',
    describe: 'URL from which to retrieve schedule',
    type: 'string'
  })
  .option('t', {
